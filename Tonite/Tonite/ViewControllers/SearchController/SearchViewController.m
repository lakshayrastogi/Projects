...
//                                   forState:UIControlStateSelected];
    ...
    _searchView.label1.text = LOCALIZED(@"Type");
    [_searchView.segmentAnimation setTitle:LOCALIZED(@"Halal") forSegmentAtIndex:0];
    [_searchView.segmentAnimation setTitle:LOCALIZED(@"Non-Halal") forSegmentAtIndex:1];
    [_searchView.segmentAnimation setTitle:LOCALIZED(@"None") forSegmentAtIndex:2];
    
    [_searchView.segmentAnimation setTintColor:THEME_ORANGE_COLOR];
...
-(void)doSearch:(NSMutableArray*) arrayFilter {
    ...
    NSArray* arrayEvents = [CoreDataController getAllEvents];
    for(Event* event in arrayEvents) {
        
        int qualifyCount = 0;
        
        
        BOOL isFoundKeyword = ([event.event_name containsString:strKeywords] ||
                               [event.event_address containsString:strKeywords] );
        ...
        if( category.length > 0) {
            ...
            MainCategory* mainCat = [CoreDataController getCategoryByCategory:category];
            
            BOOL isFoundCat = NO;
            
            if(mainCat != nil && [mainCat.category_id isEqualToString:event.category_id])
                isFoundCat = YES;
        ...
        /*
        event.distance = LOCALIZED(@"NOT_AVAILABLE");
        if(_myLocation != nil && radius > 0) {
            CLLocationCoordinate2D coord;
            coord = CLLocationCoordinate2DMake([event.lat doubleValue], [event.lon doubleValue]);
            ...
            event.distance = [NSString stringWithFormat:@"%f", distance];
        ...
        if(_searchView.segmentAnimation.selectedSegmentIndex == 0)
            index = @"1";
        
        else if(_searchView.segmentAnimation.selectedSegmentIndex == 1)
            index = @"0";
        
    
        
        if(qualifyCount == countParams)
            [arrayFilter addObject:event];
    }
