...
- (void)viewDidLoad
{
    ...
    NSString* userName = userSession.fullName;
    if(userName == nil)
        userName = [NSString stringWithFormat:@"%@-%@", LOCALIZED(@"USER"), userSession.userId];
    
    else if([userName isEqualToString:@"null"])
        userName = [NSString stringWithFormat:@"%@-%@", LOCALIZED(@"USER"), userSession.userId];
    
    _profileView.textFieldFullName.delegate = self;
    _profileView.textFieldFullName.text = userName;
    ...
    [self setImage:userSession.thumbPhotoUrl imageView:_profileView.imgViewThumb withBorder:YES isThumb:YES];
